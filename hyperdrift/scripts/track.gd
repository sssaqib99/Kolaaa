...
func _build_materials() -> void:
	...
	# These two are intentionally matte StandardMaterial3D assets: Solaris is
	# architectural and sunlit, not another emissive neon biome.
	...
	_mats["solar_edge"] = V.metal_material(Color(0.5, 0.34, 0.13), 0.75, 0.28, Color(0.7, 0.4, 0.1), 0.8)
...
func _obelisk(x: float, z: float) -> void:
	var o = _free_obstacle()
	var k := _beat_of_z(z)
	o.setup_obelisk(Vector3(x, 0.0, z), _mats["obelisk"], _mats["solar_edge"], float(k % 2))


func _dustwall(x: float, z: float, width: float = 9.0) -> void:
	var o = _free_obstacle()
	var k := _beat_of_z(z)
	o.setup_dustwall(Vector3(x, 0.0, z), width, _mats["dustwall"], _mats["solar_edge"], float(k % 2))
...
func _backfill_structure_corridor() -> void:
	...
	while z > z_lo:
		for side in [-1.0, 1.0]:
			...
			if _clashes_with_structure(bx, bz, maxf(w, bm.size.z) * 0.5, maxf(w, bm.size.z) * 0.5, h + (18.0 if _building_style == 4 else 0.0)):
				b.position.y = -500.0
				continue
...
func clear_decor_around_structure() -> void:
	...
	for b in _buildings:
		...
		var crown_top: float = b.position.y + bm.size.y * 0.5 + (18.0 if _building_style == 4 else 0.0)
...
func _decorate(z0: float) -> void:
	...
	for i in 4:
		...
		# Concert moving-head fixtures on the nearest rooftops.
		if gobo_rig != null and i < 2 and absf(b.position.x) < 60.0:
