...
## MODE, MAP, SETTINGS). SETTINGS is one compact tabbed panel (820 x 540):
...
func _build_menu() -> void:
	...
	# Main buttons: PLAY / MODE / SETTINGS
	var play := _btn(box, "▶   PLAY", Vector2(600 - 170, 372), Vector2(340, 62), V.CYAN, true, 28)
	play.pressed.connect(func() -> void: Sound.play("ui", 1.0, -6.0); game.start_run())
	_mode_btn = _btn(box, "MODE:  CLASSIC", Vector2(600 - 340, 446), Vector2(224, 46), V.YELLOW, false, 18)
	_mode_btn.pressed.connect(func() -> void: game.set_mode(1 - game.mode); Sound.play("ui", 1.1, -6.0))
	var st := _btn(box, "⚙   SETTINGS", Vector2(600 - 108, 446), Vector2(216, 46), V.VIOLET, true, 18)
	st.pressed.connect(func() -> void: show_settings(0))
	var mu := _btn(box, "♪   MUSIC", Vector2(600 + 116, 446), Vector2(224, 46), V.LIME, false, 18)
	mu.pressed.connect(func() -> void: show_settings(3))
	_menu_now = _lbl(box, "", 16, Color(0.85, 0.9, 1.0, 0.7), HORIZONTAL_ALIGNMENT_CENTER, 0, 500, 1200, 22, 4)

	_lbl(box, "A / D  STRAFE      SPACE  JUMP      S  DUCK      E  ADRENALINE      ESC  PAUSE      F2  SETTINGS", 17,
			Color(0.85, 0.9, 1.0, 0.65), HORIZONTAL_ALIGNMENT_CENTER, 0, 530, 1200, 24, 4)
	_prompt = _lbl(box, "PRESS  SPACE  TO DRIVE", 30, Color(1, 1, 1), HORIZONTAL_ALIGNMENT_CENTER, 0, 566, 1200, 44, 8)
	_menu.visible = false
...
func _build_tab_game() -> void:
	...
	_fs_check = _toggle(v, "FULLSCREEN", false, func(on: bool) -> void: if on != game.is_fullscreen(): game.toggle_fullscreen())
...
func tick(delta: float) -> void:
	...
	if settings_open:
		...
		var extra := "   •   ENERGY %d%%   TEMPO x%.2f%s" % [int(Sound.intensity * 100.0), Sound.tempo_ratio, "   •   DROP!" if Sound.drop_active else ("   •   BREAKDOWN" if Sound.breakdown else "")]
		if src == "SYNTH":
