...
func set_ship(id: int) -> void:
	Persist.ship_id = clampi(id, 0, 1)
	if player != null:
		player.set_ship(Persist.ship_id)
	Persist.save_profile()
	if hud != null:
		hud.refresh_ship()


func cycle_ship(dir: int) -> void:
	set_ship(wrapi(Persist.ship_id + dir, 0, 2))
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if hud.hangar_open:
		if event.is_action_pressed("move_left"):
			...
			hud.hide_hangar()
		elif event.is_action_pressed("music_panel") or event.is_action_pressed("pause"):
			hud.hide_hangar()
		return
