...
| Fullscreen | `F11` | – |
| Hangar (ship select) | `H` (title screen) | – |
...
**NOCTURNE BOULEVARD** is the new in-project Level B, deliberately built as an
original Art Deco noir city rather than another neon palette. It has a real wet
asphalt boulevard with dashed lane markings, a cratered full moon, low rain,
tall pastel-detailed towers with tiered crowns and spires, and safe background
sedans with warm headlights. Its soundtrack-driven grade moves slowly from
smoky crimson dusk to blue-violet midnight. Its unique beat-authored hazards
are **Noir Obelisks**, **Shadow Curtains**, and low **Noir Cabs** which sweep a
single lane while leaving broad escape routes.

## Music Direction

The Music bus now reads six live frequency bands: **sub-bass**, **bass-wave**,
**low-mids**, **melody presence**, **percussion air**, and **shimmer**. An
adaptive peak follower works with both built-in themes and device tracks, then
classifies the dominant layer as `BASS`, `DRONE`, `MELODY`, or `PERCUSSION`.

* Bass pumps the engines, hull scale, road rings, and warm gobo fans.
* Sustained drones create low hover drift, violet road breathing, and slower
  atmospheric lighting.
* Melody drives wing-side flares, blue-teal gobo sweeps, and precision glints.
* Percussion uses short spectral-flux transients for gold sparkle flares and
  rapid gobo accents.

The ship carries six texture-free radial, streak, cross, diamond, and ring
flares generated by `music_flare.gdshader`, so the craft changes with the music
instead of only bouncing to a generic beat.

## Hovercraft

Pick your craft in the **HANGAR** — a live 3D selection UI with a rotating
turntable preview built from the exact same procedural geometry you fly
(`ship_factory.gd` is shared by the game ship and the preview, so WYSIWYG).
Three ways in:

* **Title screen → HANGAR** (or press `H`): full-screen hangar overlay with a
  large live preview, `←` / `→` to browse, `ENTER` to select.
* **SETTINGS → HANGAR tab**: the same live preview docked in the settings
  panel — selection applies instantly, so you can watch your ship swap on the
  road behind the panel.
* **Title quick-select**: `←` / `→` arrows under the map name swap ships
  immediately; **GAME tab** shows the current pick with an `OPEN HANGAR →`
  shortcut.

Both craft retain the same fair collision and handling profile:

| Craft | Style |
| --- | --- |
| **VECTOR** | agile original neon interceptor, twin cyan engine pods |
| **PHANTOM** | broader original Art Deco heavy cruiser: wedge hull, swept chrome-edged wings, violet engines, exhaust spine |

Both carry the six texture-free music flares (`music_flare.gdshader`) driven by
Sound's six-band music director (bass / drone / melody / percussion), and the
hangar preview animates them live — from the actual music when a track plays,
from an idle wave otherwise.
