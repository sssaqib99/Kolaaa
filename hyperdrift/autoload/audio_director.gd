...
## * Three theme songs (one per map) are composed from scratch in code.
...
## Analysis bands (Hz) for the live beat tracker.
const BANDS := [[20.0, 60.0], [60.0, 130.0], [130.0, 300.0], [300.0, 1000.0], [1000.0, 4000.0], [4000.0, 12000.0]]
...
## Loudness / structure analysis: drives speed-follows-music, drops, gobos.
func _analyze_structure(ud: float) -> void:
	...
	if _spectrum == null or not playing:
		energy = lerpf(energy, 0.0, clampf(ud * 3.0, 0.0, 1.0))
		intensity = lerpf(intensity, 0.0, clampf(ud * 2.0, 0.0, 1.0))
		spike = 0.0
		drop_active = false
		if breakdown:
			breakdown = false
		...
		return
	var b_lin := _band_lin(20.0, 130.0)
	var m_lin := _band_lin(130.0, 1000.0)
	var h_lin := _band_lin(1000.0, 12000.0)
	var raw := b_lin * 1.6 + m_lin * 1.0 + h_lin * 1.4
	# Adaptive normaliser: slowly decaying running peak.
	energy_peak = maxf(energy_peak * (1.0 - ud * 0.04), raw)
	energy_peak = maxf(energy_peak, 0.01)
	var n := clampf(raw / (energy_peak * 0.6), 0.0, 1.0)
	var kf := clampf(ud * 7.0, 0.0, 1.0)
	energy = lerpf(energy, n, kf)
	bass = lerpf(bass, clampf(b_lin / (energy_peak * 0.62), 0.0, 1.0), kf)
	mids = lerpf(mids, clampf(m_lin / (energy_peak * 0.5), 0.0, 1.0), kf)
	highs = lerpf(highs, clampf(h_lin / (energy_peak * 0.45), 0.0, 1.0), kf)
	energy_slow = lerpf(energy_slow, energy, clampf(ud * 0.2, 0.0, 1.0))
	intensity = lerpf(intensity, energy * 0.7 + bass * 0.3, clampf(ud * 1.2, 0.0, 1.0))
	spike = clampf((energy - energy_slow) / maxf(energy_slow, 0.15) - 0.15, 0.0, 1.0)

	# Discrete spike onset (rising edge above threshold) — used by the gobo
	# rig to flash a fresh colour on each beat spike.
	...
	if breakdown and not was:
		...
		breakdown_ended.emit()
	var was_a := ambient
	# Ambient requires sustained very low energy (no kick for ~7 s) AND
	# slow energy has had time to settle. This separates "intro" / "outro"
	# quiet from "loud breakdown".
